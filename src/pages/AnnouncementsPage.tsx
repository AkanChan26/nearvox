        ...
        {showForm && (
          <div className="admin-box p-5">
            ...
            <div className="space-y-3">
              ...
              {/* Attachments */}
              <div>
                <p className="text-[10px] admin-text mb-1">&gt; ATTACHMENTS:</p>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="space-x-2">
                  <button onClick={() => { setShowForm(false); setSelectedFiles([]); }} className="text-xs text-muted-foreground hover:text-foreground">[CANCEL]</button>
                  <button onClick={handleSubmit} disabled={uploading} className="text-xs admin-text border border-admin-border px-3 py-1 hover:bg-admin hover:text-admin-foreground transition-none disabled:opacity-50">
                  ...
                  {attachments.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {attachments.map((path, i) => {
                        ...
                        if (isImage(path)) {
                          return (
                            <button key={i} onClick={() => setPreviewUrl(url)} className="relative group border border-border overflow-hidden w-16 h-16">
                        ...
                        return (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] text-foreground border border-border px-2 py-1 hover:bg-foreground/5">
