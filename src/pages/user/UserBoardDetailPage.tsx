    ...
    <div className="border border-border p-3">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="relative">
          <button onClick={() => setShowMenu(!showMenu)} className="p-1 text-muted-foreground hover:text-foreground min-h-[28px] min-w-[28px] flex items-center justify-center">
            <MoreVertical className="h-3 w-3" />
          </button>
          {showMenu && (
            <div className="absolute right-0 top-full z-10 border border-border bg-card min-w-[110px]">
              {isOwner && (
                <button onClick={() => { setEditing(true); setEditTitle(post.title || ""); setEditContent(post.content); setShowMenu(false); }}
                  className="w-full text-left text-[10px] text-foreground px-2.5 py-1.5 hover:bg-foreground/5 flex items-center gap-1.5 tracking-wider">
              ...
              {isMember && (
                <button onClick={() => { onRepost(); setShowMenu(false); }}
                  className="w-full text-left text-[10px] text-foreground px-2.5 py-1.5 hover:bg-foreground/5 flex items-center gap-1.5 tracking-wider">
                  <Copy className="h-2.5 w-2.5" /> REPOST
                </button>
              )}
              {(isOwner || isAdmin) && (
                <button onClick={() => { onDelete(); setShowMenu(false); }} className="w-full text-left text-[10px] text-destructive px-2.5 py-1.5 hover:bg-foreground/5 flex items-center gap-1.5 tracking-wider">
                  <Trash2 className="h-2.5 w-2.5" /> DELETE
                </button>
              )}
              {!isOwner && (
                <button onClick={() => { onReport(); setShowMenu(false); }} className="w-full text-left text-[10px] text-muted-foreground px-2.5 py-1.5 hover:bg-foreground/5 flex items-center gap-1.5 tracking-wider">
      ...
      {editing ? (
        <div className="space-y-2 mb-2">
          <input
            ...
            className="w-full bg-background border border-border px-2.5 py-1.5 text-[11px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-foreground/40"
          />
          <textarea
            value={editContent}
            onChange={(e) => setEditContent(e.target.value)}
            rows={3}
            className="w-full bg-background border border-border px-2.5 py-1.5 text-[11px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-foreground/40 resize-none"
          />
          <div className="flex gap-2 justify-end">
            <button onClick={() => setEditing(false)} className="text-[10px] text-muted-foreground border border-border px-2 py-1 tracking-wider">CANCEL</button>
            <button onClick={handleSaveEdit} disabled={!editContent.trim()} className="text-[10px] text-background bg-foreground px-2 py-1 tracking-wider disabled:opacity-50">SAVE</button>
          </div>
        </div>
      ) : (
        <>
      ...
      {attachments.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-2">
          {attachments.map((path: string, i: number) => {
            ...
            if (isImage(path)) {
              return (
                <button key={i} onClick={() => onPreview(url)} className="border border-border w-16 h-16 overflow-hidden group relative">
            ...
            return (
              <a key={i} href={url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[9px] text-muted-foreground border border-border px-2 py-0.5 hover:text-foreground">
      ...
      <div className="flex items-center gap-3 border-t border-border pt-1.5">
        <button onClick={onLike} className={`flex items-center gap-1 text-[10px] ${isLiked ? 'text-destructive' : 'text-muted-foreground'} hover:text-destructive`}>
          <Heart className={`h-2.5 w-2.5 ${isLiked ? 'fill-current' : ''}`} />
          {post.likes_count > 0 && post.likes_count}
        </button>
        <button onClick={onToggleComments} className="flex items-center gap-1 text-[10px] text-muted-foreground hover:text-foreground">
